...
- **覆盖率棘轮补齐并上调**（补的是 2026-09-26 之前就低于门槛的那笔账）：
  新增 `tests/virtual/water-virtual-doc.test.ts`（27 例），用 node 下的 canvas 桩驱动把
  `src/virtual/WaterVirtualDoc.ts` 从 **0%** 拉到 **98.7% 语句 / 84.17% 分支** ——
  覆盖的是这一层的几何与状态机：列存布局、网格索引（含节点数组扩容）、窗口遍历去重、
  正交路由命中、退化条目（末行自连管线）、物化让位（`done`）、编辑回流、载荷往返、
  全量 SVG 导出与 XML 转义、以及"模块加载即注册工厂"的反序列化接缝。
  全局覆盖率 84.8 / 72.48 / 87.89 / 85.45（语句 / 分支 / 函数 / 行）→
  **91.71 / 75.33 / 91.71 / 92.19**，四个门槛首次全部达标。
- `jest.config.js` 门槛按"覆盖率提升后只许上调"的规定改成 **89 / 74 / 89 / 89**：
  分支维持 74 —— 同口径该抬到 ~73，那是往下调，棘轮不许。
